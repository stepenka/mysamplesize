...
stratter <- function(sd0,sd1){

	bnds  <- setBounds();
	dfts  <- setDefaults();
	
	output <- setStrattTTest(bnds$minSampSize,bnds$maxSampSize,bnds$minES,bnds$maxES,bnds$minSD,bnds$maxSD,bnds$minSL,bnds$maxSL,dfts$ss,dfts$es,sd0,dfts$sigLevel,sd1);
...
setIndepSampTTest <- function(minSS,maxSS,minES,maxES,minSD,maxSD,minSL,maxSL,ss0,es0,sd0,sig0) {
    ...
    x <- seq(0-8/sqrt(minSS), effectSize + 8/sqrt(minSS), .005);
...
#	             tLevel=c(myParms$mu+myParms$delta,myParms$mu+myParms$delta))
 ...
 #                                   poly is the polygon data used in ggplot graphs
 #
 setStrattTTest <- function(minSS,maxSS,minES,maxES,minSD,maxSD,minSL,maxSL,ss0,es0,sd0,sig0,sd1) {
 
     yMaxValue    <- 2*dt(0,df=2);
     
     r0           <- max(sd0/sd1,sd1/sd0);
     sd0          <- min(sd0,sd1);
     
     rFact        <- (2);
     
     degFree      <- floor((ss0-1)*rFact);
     rootN        <- sqrt(ss0);
     effectSize   <- es0/sd0;
     nonCentParm  <- effectSize*rootN/(sqrt(rFact));
     tCrit        <- qt(1-sig0/2,df = degFree);  
     tCritMinus   <- 0-tCrit;
