app.service('authService', ['$http', '$q', '$state', '$uibModal', 
  function ($http, $q, $state, $uibModal) {
    ...
    this.getAccountInfo = function() {
      ...
      return $http.get('/get_stripe_account_info?user_id=' + authServ.user_id).then (
	    function successCallback(response) {
          ...
          if (data.error == false) {
            ...
            if (angular.equals(subscriptions, {}) || angular.equals(subscriptions.data, [])) {
              data.error = true;
            ...
            authServ.plan.billing = (authServ.stripeInfo.billing).replace('_', ' ');
    ...
    this.handleGetProjects = function(projDict) {
      ...
      for (var ii=0; ii<projDict.length; ii++) authServ.sqlprojects.options[ii].id = ii;
    ...
    this.getProjects = function() {
      ...
      return $http.get('/sqlprojects?user_id=' + authServ.user_id).then( function(response) {
    ...
    this.updateProfile = function(user_id, email) {
      ...
      this.email = 'bfitzpatrick@lmu.edu';        
      if (user_id != null) {
      ...
      if (user_id == null) {
        ...
        //this.email = "Guest";
        this.sqlprojects = {
		  selected: 0,options: [{id:0,name:'Login to Select a Project',created:null},],
        };            
        deferred.resolve();
      }
      ...
      $http({
	    ...
	    function successCallback(response) {
          ...
          if (authServ.authenticated) {
            ...
            return authServ.updateProfile(data.user_id, data.email).then(function(){
              def.resolve();
              return def.promise;
            });
          } else {
