...
def uid_match_required(f):
    ...
    def decorated_view(*args, **kwargs):
        ...
        if not PASS_AUTH and not isAuth:
            app_warn("redirecting_here")
            return auth_error_page()
            return authDict["message"]
        else:
...
def auth_required(f):
    ...
    def decorated_view(*args, **kwargs):
        ...
        if not PASS_AUTH and not isAuth:
            app_warn("redirectinh")
            return redirect('/logout?next=' + request.path)
            return authDict["message"]
        else:
            return f(*args, **kwargs)
    return decorated_view
...
def contact():
    ...
    msg     = Message('MySampleSize - Contact Form', sender=email, recipients=['bfitzpatrick@lmu.edu'])
    msg.body= '\nName: ' + sender + '\nEmail: ' + email + '\nPhone: ' + phone + '\n\n' + message
...
#--------------------------------------------------------#
def get_auth():
    if auth0_constants['PROFILE_KEY'] not in session or auth0_constants['JWT_PAYLOAD'] not in session:
        retval = {'authenticated': False, "message": 'You do not have permission to access this endpoint'}
    else:
        tok = session[ auth0_constants['JWT_PAYLOAD'] ]
        timenow = (datetime.now()- datetime(1970,1,1)).total_seconds()
        
        if timenow > tok["expires_in"]:
            retval = {'authenticated': False, "message": "You have been logged out and must log in again."}
            return retval
...
def callback_handling():
    ...
    expires_in_POSIX = (expires_in - datetime(1970,1,1)).total_seconds()
    tok["expires_in"] = int( expires_in_POSIX )
    
    session[auth0_constants['JWT_PAYLOAD']] = tok
