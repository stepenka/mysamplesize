...
function toolsController(
  ...
  $scope.reset = function (
    toolType,
    tmp // input is used for when embedded in DG
  ) {
    if (tmp) {
      //console.log(tmp);
    }
    ...
    if (typeof toolType !== "undefined") $scope.calculator.data.tool = toolType;
  ...
  $scope.calculate = function () {
    ...
    if (isError) {
      ...
      if (designService.isANOVA2()) nGroups += calculatorService.AOV2.k;
      ...
      else if (designService.isRMCS1()) nGroups += calculatorService.RMCS1.k;
