...
app.controller('toolsController', ['$scope', 'designService', 'calculatorService', '$http', '$q', '$uibModal', '$timeout',toolsController]);
function toolsController($scope, designService, calculatorService, $http, $q, $uibModal, $timeout)
{
...
/* Calculate Power/Sample Size and Save */
    ...
    $scope.calculate = function()
    {
        ...
        if( $scope.design.isNone() ){
            ...
            if( $scope.groupError==true||$scope.treatmentError==true||!data.tool||!data.dfM||!data.dfI||!data.dfS||!data.sig ){
                isError = true;
                console.log('err1');
            }
            
            if ($scope.calculator.data.tool=='samplesize' && (!data.k||!data.es||!data.pow)){
                isError = true;
                console.log('err2');
            }
            
            if ($scope.calculator.data.tool=='power' && (!data.df2||!data.lambda)){
                isError = true;
                console.log('err3');
            }
            ...
            if ( $scope.groupError==true||$scope.treatmentError==true||!data.tool||!data.df1 ){
                isError = true;
                console.log('err1');
                console.log( data );
            }
            ...
            $scope.calculator.RMPS1calc( $scope.calculator.data.esComb.effect, $scope.calculator.data.esComb.index );
            ...
            if ( $scope.groupError==true||$scope.treatmentError==true||!data.tool||!data.df1||!data.df2||!data.lambda){
                isError = true;
                console.log('err1');
            }
            ...
            if ( $scope.groupError==true||$scope.treatmentError==true||!data.tool||!data.sig||!data.df1||!data.lambda){
                isError = true;
                console.log('err1');
            }
            
            if ($scope.calculator.data.tool=='samplesize' && (!data.pow||!data.k )){
                isError = true;
                console.log('err2');
            }
            ...
            if ($scope.groupError==true||$scope.treatmentError==true||!data.tool||!data.es||!data.sig||!data.groups){
                isError = true;
                console.log('err1');
            }
        }
        ...
        if( isError ){
            ...
            var nGroups     = (!designService.notEmbedded) * designService.formData.nControlAdditionalSelected;
            ...
            doCalc(data).then(function(){
				...
				calculatorService.calcData = Object.assign({}, $scope.calculator.data, data);      // copy for DG report
