...
var app = angular
  .module("tempestApp", [
    ...
    "bm.uiTour",
    "nvd3",
    "rzModule", // for graphics, plotting
  ])
  .config([
    "$stateProvider",
    "$urlRouterProvider",
    ...
    function (
      ...
      $urlRouterProvider.rule(function ($injector, $location) {
        ...
        if (searchUrl == 1) $rootScope.standalone = 1;
      });

      $urlRouterProvider.otherwise(function ($injector, $location) {
        if ($location.url() === "") return $location.path("/");

        if ($location.url().length > 200) {
          //assume it's an auth0 call. need something better for this later?
  ...
  .run([
    ...
    function (
      ...
      $rootScope.trustHTML = function (src) {
