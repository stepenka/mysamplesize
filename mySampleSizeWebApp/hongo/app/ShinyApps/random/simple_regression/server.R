# server.R Simple Regression Design
  ...
  function(input, output) {
    ...
    output$block_groups = renderUI({
      lapply(1:input$block_factors, function(i) {
        selectInput(paste0('groups', i), paste0('# Groups in Additional Factor ', i),choices = c(2:20))
      })
    })
    ...
    output$title2 <- renderUI({
      HTML("<b>How many Additional Factors(eg sex, litter, or size);</b>")
    })
