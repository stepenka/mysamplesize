  ...
  function(input, output) {
    ...
    output$treat_names = renderUI({
      lapply(1:input$main_factors, function(i) {
        textInput(paste0('treat', i), paste0('Name of Dose ', i),paste0('Dose',i),width='100%',paste0('Dose',i))
      })
    })
    ...
    datum <- reactive({
      if(error() == 0){
        tryCatch({
          ...
          colnames(datum) <- c("Dose_Level",unlist(groups))
    ...
    output$error <- renderUI({
      if (error()==1){
        HTML(paste("This design has ",replicates()," replicate(s).","<br><p class='text-danger'><b>Error: Unbalanced Design</b></p><br><small>The Sample Size Per Group divided by the Number of Dose Groups must be a whole number. Please try increasing your Sample Size Per Group.</small>"))
      } else if (error()==0){
    ...
    output$title1 <- renderUI({
      HTML("<b>How many Total Dose Groups are there;</b>")
    })
