...
# sampledData_kh <- function(mu, alpha, sigma, n, p, delta, iType)
sampledData_kh <- function(muControl, sigLevel, stDev, nSubPerGp, nLevelsEachFactor, efctSizeEachEffect, iType) {
#
#  Simulate data
#
# 

myParms <- anovaPower(sigLevel,stDev,nSubPerGp,nLevelsEachFactor,efctSizeEachEffect);
...
# Turn this on if comparing Python sim calc to R
if(1){
    fid <- file("xmat.bin", "wb");
    writeBin(c(x), fid);
    close(fid);
}
...
if (nWay == 1){
    delta   <- selectResponseShape(iType,del,pv)
    pattrn  <- list("efct1" = delta )
...
if (nWay == 2) {
    ...
    myDeltaM <- selectResponseShape(iType[1], del[1], pv[1])
    myDeltaS <- selectResponseShape(iType[2], del[2], pv[2])
    myDeltaI <- selectResponseShape2(iType[3], del[3], myDeltaM, myDeltaS)
    pattrn  <- list("efct1" = myDeltaM,"efct2"=myDeltaS,"efct12"=myDeltaI)
...
### stopped here.     alf <- del[1]*(alf/sd(alf)); mu0 <- muControl - alf[1];
    ...
    myStats$pValue <- c(pVal1, pVal2, pVal12);
    ...
    #myStats$effectSize <- c(efct1e, efct2e, efct12e);
...
if (nWay == 3) {
    myDeltaM  <- selectResponseShape(iType[1],del[1],pv[1])
    myDeltaS  <- selectResponseShape(iType[2],del[2],pv[2])
    myDeltaT  <- selectResponseShape(iType[3],del[3],pv[3])
    myDeltaMS <- selectResponseShape2(iType[4],del[4],myDeltaM,myDeltaS)
    myDeltaMT <- selectResponseShape2(iType[5],del[5],myDeltaM,myDeltaT)
    myDeltaST <- selectResponseShape2(iType[6],del[6],myDeltaS,myDeltaT)

    myDeltaMST <- selectResponseShape3(pv)
    
    pattrn <- list("efct1" = myDeltaM,"efct2"=myDeltaS,"efct3"=myDeltaT,"efct12" = myDeltaMS,"efct13"=myDeltaMT,"efct23"=myDeltaST,"efct123"=myDeltaMST)
...
### stopped here.     alf <- del[1]*(alf/sd(alf)); mu0 <- muControl - alf[1];
    ...
    #myStats$fStat1  <- fStat1;
    #myStats$fStat2  <- fStat2;
    #myStats$fStat3  <- fStat3;
    #myStats$fStat12 <- fStat12;
    #myStats$fStat13 <- fStat13;
    #myStats$fStat23 <- fStat23;
    #myStats$fStat123<- fStat123;
    
    myStats$pValue <- c(pVal1, pVal2, pVal3, pVal12, pVal13, pVal23, pVal123);
